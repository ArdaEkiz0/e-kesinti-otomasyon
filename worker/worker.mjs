const H='b20b616dc4d5015c5d6a90a7d91ffabfac144e100437d8dbf72c85dd5bdcb385';
const CORS={'Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET,POST,OPTIONS','Access-Control-Allow-Headers':'Content-Type'};

const HTML='<!DOCTYPE html><html lang=tr><head><meta charset=UTF-8><meta name=viewport content="width=device-width,initial-scale=1"><title>SGK Admin Paneli</title><style>body{font-family:Arial,sans-serif;margin:20px;background:#f0f0f0}h1{color:#333}table{border-collapse:collapse;width:100%;background:#fff;margin-top:10px}th,td{border:1px solid #ccc;padding:8px;text-align:left}th{background:#222;color:#fff}button{padding:6px 12px;margin:2px;cursor:pointer;border:none;border-radius:3px}.ok{background:#4caf50;color:#fff}.no{background:#f44336;color:#fff}#stats{margin:10px 0;font-weight:bold;color:#222}input{padding:6px}</style></head><body><h1>SGK Admin Paneli</h1><div id=login><input id=pw type=password placeholder=Sifre><button onclick="login()">Giris</button><span id=msg></span></div><div id=panel style="display:none"><div id=stats></div><table id=tbl><tr><th>HWID</th><th>IP</th><th>Durum</th><th>Son Gorulme</th><th>Islem</th></tr></table></div><script>var PW="";function login(){PW=document.getElementById("pw").value;load();}function load(){fetch("/api/admin/stats?password="+encodeURIComponent(PW)).then(function(r){return r.json();}).then(function(s){if(s.error){document.getElementById("msg").textContent=s.error;return;}document.getElementById("stats").textContent="Toplam: "+s.total_users+" | Yetkili: "+s.authorized_users+" | Bekleyen: "+s.pending_users+" | 24s aktif: "+s.active_24h;}).catch(function(e){document.getElementById("msg").textContent="Hata: "+e;});fetch("/api/admin/users?password="+encodeURIComponent(PW)).then(function(r){return r.json();}).then(function(d){if(d.error){document.getElementById("msg").textContent=d.error;return;}document.getElementById("panel").style.display="block";var t=document.getElementById("tbl");for(var i=t.rows.length-1;i>0;i--){t.deleteRow(i);}d.users.forEach(function(u){var row=t.insertRow(-1);row.insertCell(0).textContent=u.hwid;row.insertCell(1).textContent=u.ip;row.insertCell(2).textContent=u.authorized?"YETKILI":"BEKLIYOR";row.insertCell(3).textContent=u.last_seen;var b=row.insertCell(4);var btn=document.createElement("button");btn.textContent=u.authorized?"Kaldir":"Yetkilendir";btn.className=u.authorized?"no":"ok";btn.onclick=function(){act(u.hwid,u.authorized?0:1);};b.appendChild(btn);});}).catch(function(e){document.getElementById("msg").textContent="Hata: "+e;});}function act(hwid,auth){fetch("/api/admin/"+(auth?"authorize":"deauthorize"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({password:PW,hwid:hwid})}).then(function(r){return r.json();}).then(function(d){if(d.error){alert(d.error);return;}load();}).catch(function(e){alert("Hata: "+e);});}</script></body></html>';
